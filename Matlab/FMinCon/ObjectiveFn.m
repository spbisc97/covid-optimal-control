...
tspan=[1 days];
%ode options max simulation step
...
if Functionals(2) ~= "-2*(sum(s).*6e-7)+sum(u)*(eye(4).*[0.3 0.2 0.4 0.1])*sum(u)'"
    Functionals(2) = "-2*(sum(s).*6e-7)+sum(u)*(eye(4).*[0.3 0.2 0.4 0.1])*sum(u)'";
end
